...
function install(){
  ...
  const hero=make('section','cosmicExplorer','cx-hero',`<div class="cx-hero-kicker">COSMIC EXPLORER // SCIENTIFIC INSTRUMENT</div><div class="cx-hero-title">NAVIGATE REALITY<br><span>BY SCALE.</span></div><div class="cx-hero-copy">Measured catalogues. Physical models. Hypotheses. One interface that tells you which is which.</div><div class="cx-hero-actions"><button id="cxLaunch" class="cx-primary">ENTER EXPLORER <span>↗</span></button><button id="cxGuide" class="cx-secondary">HOW IT WORKS</button></div>`);
  ...
  $('#cxLaunch').onclick=()=>{hero.classList.add('cx-dismiss');instrument.classList.add('open');timeline.classList.add('show');setTimeout(()=>hero.remove(),700);};
...
function searchHook(){const input=$('#searchInput');if(!input)return;input.addEventListener('keydown',e=>{if(e.key!=='Enter')return;setTimeout(()=>{const q=input.value.trim().toLowerCase();const hit=window.SOE?.searchIndex?.find(x=>x.label?.toLowerCase()===q||x.label?.toLowerCase().includes(q));if(hit&&window.CX)window.CX.openInspector(hit);},120);});}
