...
function lightTimeFromLy(ly) {
  if (!Number.isFinite(ly)) return '—';
  if (ly < 0.001) return `${fmt(ly * 365.25 * 24 * 60 * 60)} s`;
  if (ly < 1) return `${fmt(ly * 365.25)} days`;
  return `${fmt(ly)} years`;
}
function modeData(i) {
  const L = LEVELS[i];
...
function installUI() {
  ...
  function update() {
    ...
    $('#cxTitle').textContent = d.L.name;
    $('#cxScale').textContent = d.L.scaleLabel;
    $('#cxLog').textContent = `10^${d.log.toFixed(1)} m`;
    const sec = d.radius / 299792458;
    $('#cxLight').textContent = sec < 31557600 ? `${fmt(sec / 86400)} days` : `${fmt(sec / 31557600)} years`;
    $('#cxEvidence').textContent = d.L.evidence.toUpperCase();
    $('#cxSub').textContent = d.L.summary;
    if (active === 'light') {
      $('#cxRulerLabel').textContent = `A photon crosses this nominal radius in ${$('#cxLight').textContent}`;
      $('#cosmicRuler').classList.add('show');
    } else if (active === 'time') {
      $('#cxRulerLabel').textContent = d.L.evidence === 'notScience'
        ? 'No physical look-back time: this scale is undefined'
        : `Nominal radius ÷ c: ${$('#cxLight').textContent} — not a cosmological look-back time`;
      $('#cosmicRuler').classList.add('show');
      $('#cxSub').textContent = d.L.evidence === 'modelled' && d.L.id === 'observable-universe'
        ? `${d.L.summary} The displayed time is a light-crossing comparison; expansion means it is not the age/look-back time of the horizon.`
        : `${d.L.summary} TIME mode uses a simple distance ÷ speed-of-light comparison, not a cosmological model.`;
    } else if (active === 'physics') {
      $('#cxRulerLabel').textContent = `radius × 2 = ${d.L.evidence === 'notScience' ? 'undefined' : fmt(d.diameter) + ' m'}`;
      $('#cosmicRuler').classList.add('show');
    } else {
      $('#cosmicRuler').classList.remove('show');
    }
  }
  function setMode(m) {
    active = m;
    document.body.classList.remove('cx-warp', 'cx-time', 'cx-map');
    if (m === 'light') document.body.classList.add('cx-warp');
    if (m === 'time') document.body.classList.add('cx-time');
    if (m === 'map') document.body.classList.add('cx-map');
    dock.querySelectorAll('.cx-mode').forEach(b => b.classList.toggle('active', b.dataset.mode === m));
    mode.textContent = { nav: 'NAVIGATION', light: 'LIGHT-CROSSING MODE', time: 'LOOK-BACK TIME', map: 'COSMIC MAP', physics: 'PHYSICS SCALE' }[m];
    mode.classList.remove('flash'); void mode.offsetWidth; mode.classList.add('flash');
    telemetry.classList.add('open');
    update();
  }
  dock.querySelectorAll('.cx-mode').forEach(b => b.onclick = () => setMode(b.dataset.mode));
  const ladder = $('#ladder');
  if (ladder) new MutationObserver(update).observe(ladder, { subtree: true, attributes: true, attributeFilter: ['class'] });
  addEventListener('keydown', (e) => {
    if (document.activeElement?.tagName === 'INPUT') return;
    const modes = { '1': 'nav', '2': 'light', '3': 'time', '4': 'map', '5': 'physics' };
...
function installParticles() {
  const start = () => {
    const soe = window.SOE;
    if (!soe?.scene) return setTimeout(start, 500);
    ...
    for (let i = 0; i < count; i++) {
      const r = 110 + Math.random() * 900, a = Math.random() * Math.PI * 2, z = (Math.random() * 2 - 1) * r * .35;
      p[i * 3] = Math.cos(a) * r; p[i * 3 + 1] = z; p[i * 3 + 2] = Math.sin(a) * r;
    }
    ...
    if (!reduced) {
      const tick = () => { cloud.rotation.y += .00009; cloud.rotation.x += .000015; requestAnimationFrame(tick); };
      tick();
    }
  };
  start();
...
function installSearchIntelligence() {
  ...
  input.addEventListener('keydown', (e) => {
    ...
    setTimeout(() => {
      ...
      const hit = window.SOE.searchIndex.find(e => e.label.toLowerCase() === q || e.label.toLowerCase().includes(q));
      if (hit?.payload) {
        const p = hit.payload;
        const ly = p.dist_ly ?? (p.dist_mly != null ? p.dist_mly * 1e6 : null);
        const t = ly != null ? lightTimeFromLy(ly) : 'not directly defined';
        $('#cxSub').textContent = `${hit.blurb}  Distance ÷ c: ${t}. This is a simple light-travel comparison; cosmological look-back time requires additional cosmological parameters.`;
